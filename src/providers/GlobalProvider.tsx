      ...
      (async () => {
        try {
          ...
          const songPromise = axios.get(
            `${jioApi}/api/search/songs?query=` + text,
          );

          const albumPromise = axios.get(
            `${jioApi}/api/search/albums?limit=30&&query=` + text,
          );
          const playlistPromise = axios.get(
            `${jioApi}/api/search/playlists?limit=30&&query=` + text,
          );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const songPromise = axios.get(
          `${jioApi}/api/search/songs?query=` + "arijit",
        );

        const albumPromise = axios.get(
          `${jioApi}/api/search/albums?limit=30&&query=` + "arijit",
        );
        const playlistPromise = axios.get(
          `${jioApi}/api/search/playlists?limit=30&&query=` + "arijit",
        );
  ...
  const handleSingleAlbumOrPlalist = useCallback(
    async (id: string, type: string) => {
      try {
        ...
        let url = "";
        if (type === "album") {
          url = `${jioApi}/api/albums?limit=50&&id=` + id;
        }
        if (type === "playlist") {
          url = `${jioApi}/api/playlists?limit=50&&id=` + id;
        }
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data }: any = await axios.get(
        `${jioApi}/api/search/songs?limit=50&query=${searchQuery}&page=${page + 1}`,
      );
      setPage(page + 1);
